...
void WriteStringToEEPROM(int beginaddress, String string)
{
	...
	for (uint8_t t = 0; t < sizeof(charBuf); t++)
	{
...
void EEPROMWriteFloat(int ee, double value)
{
	byte *p = (byte *)(void *)&value;
	for (int i = 0; i < sizeof(value); i++)
		EEPROM.write(ee++, *p++);
}
float EEPROMReadFloat(long ee)
{
	double value = 0.0;
	byte *p = (byte *)(void *)&value;
	for (int i = 0; i < sizeof(value); i++)
...
void addTime(byte *time1, byte *time2, byte *addedTime)
{
	addedTime[1] = time1[1] + time2[1] - ((time1[1] + time2[1]) / 60) * 60;
	addedTime[0] = time1[0] + time2[0] + (time1[1] + time2[1]) / 60 - (((time1[0] + time2[0] + (time1[1] + time2[1]) / 60)) / 24) * 24;
}

void subtractTime(byte *time1, byte *time2, byte *subtractedTime)
{
	int _time1 = time1[1] + time1[0] * 60;
	int _time2 = time2[1] + time2[0] * 60;
	int _subtractedTime = _time1 - _time2;
	subtractedTime[1] = _subtractedTime - (_subtractedTime / 60) * 60;
	subtractedTime[0] = _subtractedTime / 60 - (((_subtractedTime / 60)) / 24) * 24;
}
