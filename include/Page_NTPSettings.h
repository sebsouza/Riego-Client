...
void send_NTP_configuration_html()
{
	...
	{
		networkConfig.daylight = false;
		String temp = "";
		for ( uint8_t i = 0; i < server.args(); i++ ) {
			if (server.argName(i) == "ntpserver") networkConfig.ntpServerName = urldecode( server.arg(i)); 
			if (server.argName(i) == "update") networkConfig.Update_Time_Via_NTP_Every =  server.arg(i).toInt(); 
			if (server.argName(i) == "tz") networkConfig.timezone =  server.arg(i).toInt(); 
			if (server.argName(i) == "dst") networkConfig.daylight = true; 
		}
		Serial.println("Time Zone set to: "+(String)networkConfig.timezone);
		writeConfig();
		firstStart = true;
	}
...
void send_NTP_configuration_values_html()
{
	...
	String values ="";
	values += "ntpserver|" + (String) networkConfig.ntpServerName + "|input\n";
	values += "update|" +  (String) networkConfig.Update_Time_Via_NTP_Every + "|input\n";
	values += "tz|" +  (String) networkConfig.timezone + "|input\n";
	values += "dst|" +  (String) (networkConfig.daylight ? "checked" : "") + "|chk\n";
